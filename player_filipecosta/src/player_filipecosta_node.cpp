...
public:
  ...
  bool getLimits()
  {
    ...
    try
    {
      listener.waitForTransform("filipecosta", "world", now, Duration(DEFAULT_TIME));
      listener.lookupTransform("filipecosta", "world", now, trans);
    }
    catch (tf::TransformException ex)
    {
      ROS_ERROR("%s", ex.what());
  ...
  void move(const rws2018_msgs::MakeAPlay::ConstPtr &msg)
  {
    ...
    marker.header.frame_id = "filipecosta"; // referencial name
    ...
    br.sendTransform(
        tf::StampedTransform(transform, ros::Time::now(), "world", "filipecosta")); // constroi e envia a mensagem
...
int main(int argc, char **argv)
{
  ...
  if (my_player.red_team->playerBelongsToTeam("filipecosta"))
  {
    ...
    ROS_INFO("Filipe is in the correct team.");
  };
